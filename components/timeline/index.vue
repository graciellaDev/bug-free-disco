<template>
  <div ref="timelineContainer" class="max-h-[calc(100vh-230px)] overflow-y-auto bg-athens-gray p-25px pb-2.5 pt-70px">
    <template v-for="(group, index) in timelineGroups" :key="index">
      ...
      <template v-for="event in group.events" :key="event.id">
        <!-- Системные события -->
        <p v-if="event.type === 'system'" class="px-20 pb-0.5 text-13px font-normal text-slate-custom">
          <span>{{ event.time }}</span>
          {{ event.content }}
        </p>

        <!-- Пропущенные звонки -->
        <TimelineCall v-else-if="event.type === 'call'" :calls="event.calls" @complete="handleComplete" />

        <!-- Заметки -->
        <TimelineNote v-else-if="event.type === 'note'" :note="event" />

        <!-- Задачи -->
        <TimelineTask v-else-if="event.type === 'task'" :task="event" @complete="handleComplete" />

        <!-- Email -->
        <TimelineEmail v-else-if="event.type === 'email'" :emails="event.emails" @send="handleSendEmail" />

        <!-- Сообщения локальные, работных сайтов и мессенджеров -->
        <TimeLineMessage v-else-if="event.type === 'chat' || event.type === 'messanger'" :event="event"
        ...
        <!-- Комментарии -->
        <TimelineComment v-else-if="event.type === 'comment'" :comment="event" @reply="handleReplyInThread" />
      </template>
    </template>
  </div>
</template>
...
import TimeLineMessage from './items/TimeLineMessage.vue';
...
const handleReplyInTelegram = message => {
  emit('reply', { type: 'messanger', message });
};
