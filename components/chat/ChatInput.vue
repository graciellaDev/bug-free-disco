<template>
  <div class="w-full bg-white rounded-b-fifteen relative" :class="{ 'z-10': isExpanded || isFocused }"
    @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave">
    <!-- Расширяющаяся часть (прикрепленные файлы и текстовое поле) -->
    <Transition name="slide-down">
      <div v-if="isExpanded || message || attachments.length > 0"
        ...
        <div class="p-15px pt-25px">
          <textarea placeholder="Введите текст..." v-model="message"
            class="w-full resize-none bg-transparent outline-none text-sm text-space font-normal"
            :class="{ 'max-h-[200px] overflow-y-auto': hasOverflow }" :style="{ height: textareaHeight }"
    ...
    <div class="h-[61px] flex flex-col justify-between">
      ...
      <div class="px-15px pt-15px flex items-center transition-all duration-300" :style="{
        transform:
          isExpanded || message || attachments.length > 0
            ? `translateY(-${expandHeight}px)`
            : 'translateY(0)',
      }">
        <ChatFormatSelector v-model="selectedFormat" @select="handleFormatSelect" />
        ...
        >
          {{ recipient }} -->
        <!-- </button> -->
        <span v-if="!isExpanded && !message" class="text-sm text-slate-custom font-normal ml-2">
          введите текст...
        </span>
      </div>
      ...
      <Transition name="fade">
        <div v-if="showControls" class="px-15px flex items-center gap-2" style="transform: translateY(-35px)">
          <UiButton class="font-semibold" variant="action" size="action" @click="handleSend">
            Отправить
          </UiButton>
          <UiButton class="font-medium mr-auto" variant="back" size="back" @click="handleCancel">
            Отменить
          </UiButton>
          <div class="relative">
            ...
            <button
              ...
              @click="triggerFileInput" :class="{ 'bg-space text-white': attachments.length > 0 }">
