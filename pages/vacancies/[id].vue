...
const candidatesCountByStage = computed(() => {
  ...
  stages.value.forEach((s: Stage) => {
    counts[s.id] = 0;
  });

  if (candidatesList.value) {
    candidatesList.value.forEach((c: Candidate) => {
...
const filteredVacancies = computed(() => {
  ...
  return vacancies.value
    .filter((v: Vacancy) => v.id !== vacancy.value?.id)
    .map(v => ({
      name: v.title || v.name,
      value: v.id,
    }));
...
const filteredCandidatesList = computed(() => {
  ...
  return candidatesList.value.filter(
    (candidate: Candidate) => candidate.stage === selectedStageId.value
  );
});
...
<template>
  <div class="container pt-35px">
    <div class="relative rounded-t-fifteen bg-white p-25px">
      <div class="flex items-center justify-between">
        <div class="flex flex-col gap-2.5">
          <div class="flex flex-col gap-2.5">
            <div class="relative">
              <div class="flex cursor-pointer items-center gap-2 text-xl font-semibold leading-normal text-space"
                @click="isDropdownOpen = !isDropdownOpen">
                <span>{{ vacancy?.name || 'Выберите вакансию' }}</span>
                <svg-icon name="dropdown-arrow" width="20" height="20" :class="{ 'rotate-180': isDropdownOpen }"
              ...
              <transition name="slide-fade">
                <div v-if="isDropdownOpen"
                  ...
                  <div v-for="v in filteredVacancies" :key="v.value"
        ...
        <UiButton v-if="userRole === 'admin'" size="semiaction" variant="action" @click="addCandidatePopup.open()">
          Добавить кандидата
        </UiButton>
      </div>
    </div>
    <div
      ...
      <button class="flex cursor-pointer gap-x-2.5 rounded-ten px-15px py-2.5 text-sm font-medium"
        @click="handleClickAll()" style="
          transition-property: background-color, color;
          transition-duration: 0.2s;
          transition-timing-function: ease-in-out;
        " :class="isActiveAll ? 'bg-space text-white' : 'bg-transparent text-space'
          ">
        <p>Все</p>
      ...
      <button v-for="(stage, index) in stages" :key="stage.id"
        class="flex cursor-pointer gap-x-2.5 rounded-ten px-15px py-2.5 text-sm font-medium text-space"
        @click="handleStageClick(stage.id)" :class="selectedStageId === stage.id
          ? 'bg-space text-white'
    ...
    <div v-else>
      <div v-if="filteredCandidatesList.length > 0" class="flex gap-x-15px">
        <div class="w-[375px] rounded-sixteen bg-white">
          <CandidateList :candidates="filteredCandidatesList || []" :selected="selected" :show-checkboxes="true"
            :all-selected="allSelected" :loading="loadingCandidates" @item-click="handleCandidateClick"
        ...
        <div v-else-if="selectedCandidate" class="w-full">
          <BlockCandidateInfo :candidate="selectedCandidate" :stages="stages" :isFunnel="true" :vacancy="vacancy"
            @candidate-updated="handleCandidateUpdated" @candidate-moved="handleCandidateMoved"
    ...
    <div v-if="userRole === 'admin'">
      <CandidateAddPopup :isOpen="addCandidatePopup.isOpen" :model-value="candidateFormData"
        :server-errors="serverErrors" :loading="isSubmitting" :is-success="isSuccess" :success-message="successMessage"
        @close="addCandidatePopup.close" @submit="handleFormSubmit" @cancel="handleFormCancel"
